#include <bits/stdc++.h>
using namespace std;

struct TreeNode {
    long long val;
    TreeNode *left, *right;
    TreeNode(long long x) : val(x), left(nullptr), right(nullptr) {}
};

TreeNode* buildTree(const vector<string>& nodes) {
    if (nodes.empty() || nodes[0] == "null") return nullptr;

    TreeNode* root = new TreeNode(stoll(nodes[0]));
    queue<TreeNode*> q;
    q.push(root);

    int i = 1;
    while (!q.empty() && i < nodes.size()) {
        TreeNode* cur = q.front();
        q.pop();

        if (nodes[i] != "null") {
            cur->left = new TreeNode(stoll(nodes[i]));
            q.push(cur->left);
        }
        i++;

        if (i < nodes.size() && nodes[i] != "null") {
            cur->right = new TreeNode(stoll(nodes[i]));
            q.push(cur->right);
        }
        i++;
    }
    return root;
}
bool treeSearch(TreeNode *root, long long x) {
    //base case
    if (root == NULL) return false; 
    if (root -> val == x) return true; 

    // bool leftAns = treeSearch(root -> left, x);
    // if (leftAns == true) return true; 

    // bool rightAns = treeSearch(root -> right, x);
    // return rightAns == true;

    return (treeSearch(root -> left, x) || treeSearch(root -> right, x)); 
}

// ================= STUDENT FUNCTION =================
bool searchValue(TreeNode* root, long long X) {
    // write your code here
    bool ans = treeSearch(root, X); 

    return ans; 
}
// ====================================================

int main() {
    ios::sync_with_stdio(false);
    cin.tie(nullptr);

    int n;
    cin >> n;

    vector<string> nodes(n);
    for (int i = 0; i < n; i++) cin >> nodes[i];

    long long X;
    cin >> X;

    TreeNode* root = buildTree(nodes);

    cout << (searchValue(root, X) ? "true" : "false") << '\n';
}